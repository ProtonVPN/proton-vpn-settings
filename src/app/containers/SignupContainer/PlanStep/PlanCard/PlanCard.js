...
const PlanCard = ({ plan, isActive, onSelect, cycle, currency, isDisabled }) => {
    const button = (
        <Button
            disabled={isDisabled}
            onClick={onSelect}
            className={classnames(['w100 increase-surface-click mtauto', isActive && 'pm-button--primary'])}
        >
            ...
            <div className="p1 flex flex-items-center">
                ...
                {plan.isBest && (
                    <div className="mlauto">
                        <Badge type="primary">{c('Plan info').t`Most popular`}</Badge>
                    </div>
                )}
            </div>
            <div className="flex-item-fluid-auto p1 flex cursor-pointer flex-column">
